...
def initializer(finish,start,decrement=-1):
    ...
    today=date_-datetime.timedelta(days=0)
    file_name=str(today)[5:10]+shift[0]+'.xlsx'
    try:
        pd.read_excel('archive/'+file_name)
    except:
...
#gets data for sorting and putaway    
def putaway_get_data():
    ...
    out_path='archive\putaway_database.xlsx'
    writer = pd.ExcelWriter(out_path , engine='xlsxwriter')
    df.to_excel(writer, sheet_name='Sheet1')
    writer.save()
    return df

#returns the table for putaway and sorting
